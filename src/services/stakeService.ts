import { CONTRACT_ADDRESSES, CONTRACT_ADDRESSES_HEX, TOKEN_ADDRESSES_HEX } from '../config/contracts';
...
export async function addStake(contractAddress, senderAddress, network, tokenSymbol, amount) {
  ...
  if (tokenSymbol === 'BTC') {
    throw new Error('BTC stakes are not supported by the OP20 stake contract');
  }
