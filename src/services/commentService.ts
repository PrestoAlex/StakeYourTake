...
export async function getCommentContract(contractAddress, abi, network, senderAddress?) {
  if (!contractAddress) {
...
export async function addComment(contractAddress, senderAddress, network, commentText) {
  if (!senderAddress) {
  ...
  try {
    ...
    const contract = await getCommentContract(contractAddress, COMMENT_COUNTER_ABI, network, senderAddressObj);
...
export class CommentService {
  ...
  constructor() {
    // Use like contract address for testing since comment contract is not deployed
  ...
  async addComment(commentText: string = 'Default comment'): Promise<string | null> {
    ...
    try {
      const result = await addComment(this.contractAddress, this.senderAddress, this.network, commentText);
