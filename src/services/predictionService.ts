...
// ABI для PredictionStorage
export const PREDICTION_STORAGE_ABI = [
  {
    type: 'function',
...
export async function getPredictionContract(contractAddress, abi, network) {
  if (!contractAddress) {
    throw new Error('Prediction contract address is not configured');
  }
  ...
  const typedAbi = normalizeAbi(abi || PREDICTION_STORAGE_ABI, ABIDataTypes, BitcoinAbiTypes);
...
export async function createPrediction(contractAddress, senderAddress, network) {
  if (!senderAddress) {
    throw new Error('Wallet address required');
  }

  const contract = await getPredictionContract(contractAddress, PREDICTION_STORAGE_ABI, network);
  
  if (typeof contract.createPrediction !== 'function') {
    throw new Error('Method createPrediction not found on contract');
  }

  console.log('🚀 Creating prediction...');

  try {
    const simulation = await contract.createPrediction();
    ...
    console.log('✅ Prediction created:', txid);
    
    return {
      ok: true,
      txid,
      predictionId: Date.now().toString(), // Timestamp як ID
...
export async function getPredictionCount(contractAddress, network) {
  const contract = await getPredictionContract(contractAddress, PREDICTION_STORAGE_ABI, network);
...
export class PredictionService {
  ...
  constructor() {
    this.contractAddress = CONTRACT_ADDRESSES.prediction;
  ...
  async createPrediction(): Promise<string | null> {
    ...
    try {
      const result = await createPrediction(this.contractAddress, this.senderAddress, this.network);
  ...
  async getPredictionCount(): Promise<number> {
    try {
      const result = await getPredictionCount(this.contractAddress, this.network);
      return result.count;
    } catch (error) {
